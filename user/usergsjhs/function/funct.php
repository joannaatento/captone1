<?php
    ...
    if(isset($_POST['submit_data'])){ // pag get ng data 
        ...
        $bothparents = trim(mysqli_real_escape_string($conn, $_POST['bothparents']));
        $livesfather = trim(mysqli_real_escape_string($conn, $_POST['livesfather']));
        $livesmother = trim(mysqli_real_escape_string($conn, $_POST['livesmother']));
        ...
        $altrelation = trim(mysqli_real_escape_string($conn, $_POST['altrelation']));
        ...
        $sql = "INSERT INTO healthrecordformgsjhs VALUES ('','$user_id','$image','$gradelevel','$role','$fullname','$idnumber','$age',
        '$gender','$address','$paddress','$father','$cfather','$mother','$cmother','$religion','$nationality','$language','$bothparents',
        '$livesfather','$livesmother','$guardian','$guardianrelation','$cguardian','$altrelation','$altrel','$acontact','$bcg',
      ...
      if (mysqli_query($conn, $sql)) {
        if (move_uploaded_file($_FILES["image"]["tmp_name"], "/xampp/htdocs/CAPSTONE1/upload_image/" . $_FILES["image"]["name"]) &&
            move_uploaded_file($_FILES["imagevac"]["tmp_name"], "/xampp/htdocs/CAPSTONE1/upload_image/" . $_FILES["imagevac"]["name"])) {
            ...
            // There was an error uploading one or both files
            if (!$imageUploadSuccess) {
                echo "Error uploading the first image: " . $_FILES["image"]["error"];
            }
            if (!$secondFileUploadSuccess) {
