<?php
    ...
    if (isset($_POST['submit_medical'])) {
        ...
        if ($checkResult) {
            ...
            if ($row['count'] == 0) {
                // No duplicate entry, proceed with the insertion
                $sql = "INSERT INTO medicalapp (user_id, idnumber, fullname, gradecourseyear1, phoneno, date_time, sched_time, role, onoff, created_at, is_deleted_on_website, status) 
...
if (isset($_POST['submit_physician'])) {
    ...
    if ($checkResult) {
        ...
        if ($row['count'] == 0) {
            // No duplicate entry, proceed with the insertion
            $sql = "INSERT INTO physicianapp (user_id, idnumber, fullname, gradesection, phoneno, date_time, sched_time, role, created_at, is_deleted_on_website, status) 
            ...
            if (mysqli_query($conn, $sql)) {
                ...
                $_SESSION['success'] = "
                    <div id='success-message' style='position:absolute; right:30px; background-color:#15a362; padding: 10px 10px; width:auto; border-radius: 10px;'>
                        <h2 style='color: #fff; font-size: 16px; margin-left: 10px;'>Physician Consultation Request Appointment Submitted.</h2>
                    </div>
                ";
            }
        } else {
